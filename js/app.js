(() => {
    ...
    (() => {
        ...
        $(document).ready((function() {
            ...
            if ($(".profile__header-sl").length > 0) $(".profile__header-sl").slick({
                slidesToShow: 6,
                slidesToScroll: 1,
                rows: 1,
                slidesPerRow: 1,
            ...
            if ($(".order-table").length > 0) $(".order-table").mCustomScrollbar({
                theme: "dark-3",
                axis: "x"
            });
            if ($(".basket-registration__tab-list").length > 0) $("ul.basket-registration__tab-list li").click((function() {
                var tab_id = $(this).attr("data-tab");
                $("ul.basket-registration__tab-list li").removeClass("current");
                $(".basket-registration__tab-content").removeClass("current");
                $(this).addClass("current");
                $("#" + tab_id).addClass("current");
            }));
            $(".modal-alert__close").click((function() {
                $(this).closest(".modal-alert").removeClass("visible");
            }));
            $("body").on("click", ".form-password__control", (function() {
                if ($(this).closest(".form-password").find(".password-input").attr("type") == "password") {
                    $(this).addClass("view");
                    $(this).closest(".form-password").find(".password-input").attr("type", "text");
                } else {
                    $(this).removeClass("view");
                    $(this).closest(".form-password").find(".password-input").attr("type", "password");
                }
                return false;
            }));
            if ($(".menu__close").length > 0) $(".menu__close").on("click", (function() {
                $(this).closest(".menu").toggleClass("js-open");
                $(this).closest(".container").find(".content-wrap").toggleClass("js-menu-open");
            }));
            if ($(window).width() >= 992 && $(".header-st__profile").length > 0) {
                $(".header-st__profile").mouseover((function() {
                    $(this).find(".header-st__profile-desc").stop().addClass("js-open");
                    $(this).closest(".content-wrap").stop().addClass("js-profile-open");
                }));
                $(".header-st__profile").mouseleave((function() {
                    $(this).find(".header-st__profile-desc").stop().removeClass("js-open");
                    $(this).closest(".content-wrap").stop().removeClass("js-profile-open");
                }));
            }
            if ($(window).width() <= 991 && $(".header-st__menu-toggle").length > 0) {
                $(".header-st__menu-toggle").click((function() {
                    $("body").addClass("js-mobmenu-open");
                    $("body").find(".menu").addClass("js-mobmenu-open");
                }));
                $(".menu__close-menu").click((function() {
                    $("body").removeClass("js-mobmenu-open");
                    $("body").find(".menu").removeClass("js-mobmenu-open");
                }));
                jQuery((function($) {
                    $(document).mouseup((function(e) {
                        var divmenu = $(".menu");
                        if (!divmenu.is(e.target) && divmenu.has(e.target).length === 0) {
                            divmenu.closest("body").removeClass("js-mobmenu-open");
                            divmenu.removeClass("js-mobmenu-open");
                        }
                    }));
                }));
            }
            function onResize() {
                var el = $(".progress-c__photo");
                el.height(el.width());
            }
            ...
            if ($(".faq__item-title").length > 0) !function(i) {
                var o, n;
                i(".faq__item-title").on("click", (function() {
                    o = i(this).parents(".faq__item"), n = o.find(".faq__item-content"), o.hasClass("active_block") ? (o.removeClass("active_block"), 
                    n.slideUp()) : (o.addClass("active_block"), n.stop(!0, !0).slideDown(), o.siblings(".active_block").removeClass("active_block").children(".faq__item-content").stop(!0, !0).slideUp());
                }));
            }(jQuery);
        }));
        ...
        function initSliders() {
            if (document.querySelector(".statistics__body")) new Swiper(".statistics__body", {
                ...
                speed: 300,
                pagination: {
                    el: ".works-dashboard__pagination",
                    type: "progressbar"
                },
                navigation: {
                    prevEl: ".works-dashboard__arrow-prev",
                    nextEl: ".works-dashboard__arrow-next"
                },
                breakpoints: {
                    475: {
                        slidesPerView: 2.5
        ...
        function initSwiper() {
            ...
            } else if (!window.swiperInstance) window.swiperInstance = new Swiper(".works-dashboard__body", {
                observer: true,
                observeParents: true,
                slidesPerView: 1.8,
                spaceBetween: 15,
        ...
        window.addEventListener("DOMContentLoaded", (() => {
            const hourSelector = document.querySelector("[data-time-hour]");
            const minuteSelector = document.querySelector("[data-time-minute]");
            const secondSelector = document.querySelector("[data-time-second]");
            document.querySelector("[data-timer-start-value]");
            if (hourSelector && minuteSelector && secondSelector) {
                document.addEventListener("selectCallback", (function(e) {
                    const currentSelect = e.detail.select;
                    currentSelect.closest(".medit__select").querySelector("input").value = currentSelect.value < 10 ? `0${currentSelect.value}` : currentSelect.value;
                }));
                const allTags = document.querySelectorAll("[data-time-tag]");
                if (allTags) allTags.forEach((item => {
                    item.addEventListener("click", (e => {
                        e.preventDefault();
                        setTime(hourSelector, minuteSelector, secondSelector, parseInt(item.getAttribute("data-time-tag")));
                    }));
                }));
            }
            function setTime(selectorHour, selectorMinute, selectorSecond, secondsValue) {
                const {hours, minutes, seconds} = timeNormalize(secondsValue);
                if (secondsValue === 0) {
                    selectorHour.value = "00";
                    selectorMinute.value = "00";
                    selectorSecond.value = "00";
                    return;
                }
                selectorHour.value = hours < 10 ? `0${hours}` : hours;
                selectorMinute.value = minutes < 10 ? `0${minutes}` : minutes;
                selectorSecond.value = seconds < 10 ? `0${seconds}` : seconds;
            }
            function setTimeUp(selector, secondsValue) {
                let {hours, minutes, seconds} = timeNormalize(secondsValue);
                hours = hours < 10 ? `0${hours}` : hours;
                minutes = minutes < 10 ? `0${minutes}` : minutes;
                seconds = seconds < 10 ? `0${seconds}` : seconds;
                selector.textContent = `${hours}:${minutes}:${seconds}`;
            }
            const resetButton = document.querySelector("[data-reset]");
            if (resetButton) resetButton.addEventListener("click", (() => {
                resetTime(hourSelector, minuteSelector, secondSelector);
            }));
            function resetTime(sel1, sel2, sel3) {
                if (sel1 && sel2 && sel3) {
                    sel1.value = "";
                    sel2.value = "";
                    sel3.value = "";
            ...
            let pauseFinishTime = 0;
            const btnStop = document.querySelector("[data-timer-stop]");
            if (btnStop) {
                const parseValue = parseInt(btnStop.dataset.timerStop);
                if (parseValue === 0) {
                    btnStop.closest(".medit").classList.add("stop");
                    pause = true;
                }
                btnStop.addEventListener("click", (e => {
                    e.preventDefault();
                    e.target.closest(".medit").classList.add("stop");
                    pause = true;
                    pauseStartTime = (new Date).getTime();
                }));
            }
            const btnPlayPopup = document.querySelector("[data-popup-btn]");
            if (btnPlayPopup) btnPlayPopup.addEventListener("click", (e => {
                e.preventDefault();
                document.querySelector(".medit").classList.remove("stop");
                pause = false;
                pauseFinishTime = (new Date).getTime();
            }));
            const btnPlay = document.querySelector("[data-timer-play]");
            if (btnPlay) btnPlay.addEventListener("click", (e => {
                e.preventDefault();
                e.target.closest(".medit").classList.remove("stop");
                pause = false;
                pauseFinishTime = (new Date).getTime();
            }));
            const countDown = seconds => {
                let secondsFix = parseInt(seconds);
                let x = setInterval((function() {
                    setTime(hourSelector, minuteSelector, secondSelector, secondsFix);
                    if (!pause) {
                        let now = (new Date).getTime();
                        let distance = now + secondsFix - now;
                        if (distance === 0) {
                            clearInterval(x);
                            setTime(hourSelector, minuteSelector, secondSelector, 0);
                            const upTime = document.querySelector("[data-time-up]");
                            const upTimeValue = document.querySelector("[data-time-up-value]");
                            if (upTime) {
                                upTime.hidden = false;
                                upTime.closest(".medit").classList.add("finish");
                                const now = (new Date).getTime();
                                const audio = document.querySelector(".audio-timer");
                                if (audio) audio.play();
                                setInterval((() => {
                                    let nowInterval = (new Date).getTime();
                                    let timeLeft = 0;
                                    timeLeft = Math.ceil((nowInterval - now - 1e3) / 1e3);
                                    const twoTimer = document.querySelector("[data-time-two]");
                                    if (twoTimer) twoTimer.value = timeLeft;
                                    setTimeUp(upTimeValue, timeLeft);
                                }), 1e3);
                            }
                        }
                        setTime(hourSelector, minuteSelector, secondSelector, distance);
                        secondsFix -= 1;
                    }
                }), 1e3);
            };
            const twoTimer = document.querySelector("[data-time-two]");
            if (twoTimer && twoTimer.value > 0) {
                const sec2 = parseInt(twoTimer.value);
                const upTime = document.querySelector("[data-time-up]");
                const upTimeValue = document.querySelector("[data-time-up-value]");
                if (upTime) {
                    upTime.hidden = false;
                    ...
                    setInterval((() => {
                        let nowInterval = (new Date).getTime();
                        let timeLeft = 0;
                        timeLeft = Math.ceil((nowInterval - now + sec2 * 1e3 + -1e3) / 1e3);
                        const twoTimer = document.querySelector("[data-time-two]");
                        if (twoTimer) twoTimer.value = timeLeft;
                        setTimeUp(upTimeValue, timeLeft);
                    }), 1e3);
            ...
            function timeNormalize(secondsValue) {
                let hours, minutes, seconds;
                const totalMinutes = Math.floor(secondsValue / 60);
            ...
            const dataLeft = document.querySelector("[data-time-left]");
            if (dataLeft) {
                const timeLeft = dataLeft.value;
                if (timeLeft > 0) countDown(timeLeft);
            }
        }));
        const profileBlock = document.querySelector(".header-st__profile");
        const body = document.body;
        if (profileBlock) {
            profileBlock.addEventListener("mouseover", (function() {
                body.classList.add("_hover");
            }));
            profileBlock.addEventListener("mouseout", (function() {
                body.classList.remove("_hover");
            }));
        }
        window["FLS"] = false;
        isWebp();
